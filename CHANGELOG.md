...
- Upgraded `concurrently` to `10.0.5` (now ESM-only, requires Node >=22); the existing `dev` script's `-k`/kill-others flag behavior is unchanged. Verified `npm run dev` boots both the API and Vite dev server, plus a clean build, lint, and Playwright e2e pass.
- Upgraded Vite to `8.3.0` (now powered by the Rolldown bundler) and `@vitejs/plugin-react` to `6.1.1`. Vite 8 removes `rollup` as a direct dependency in favor of re-exporting Rollup-compatible types from `vite` itself, so `vite.config.ts`'s custom bundle-manipulation plugin now imports `type { Rollup }` from `vite` instead of `rollup` (type-only change, no behavior change). Verified with a clean build, lint, the full Playwright e2e suite, and a manual `vite preview` smoke check confirming the custom critical-CSS/font-preload/route-modulepreload HTML injections are all still emitted correctly; production build time also dropped noticeably (~1.2s to ~0.7s).
- Applied the latest in-range ESLint 9.x patch release (`9.39.5`).
